...
1. 游戏窗口仅支持16:9分辨率,推荐分辨率为1920×1080或1280x720,屏幕缩放为100%
...
7. ### 定时重启
   config变量控制截窗口失败和定时重启避免脚本崩溃及游戏卡加载
   ```shell
   RebootScreenshot: false # 是否在游戏窗口存在但截取失败后重启
   ReStartWutheringWavas: false # 为避免游戏卡10%、75%等特殊进度是否定时重启游戏
   ReStartWutheringWavasTime: 7200 # 单位为秒，如7200秒且ReStartWutheringWavas为True即两小时重启一次
   ```
